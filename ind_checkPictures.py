...
def formatTempTxt(temp):
    ...
    if 10 <= temp < 100:
        ...
        return(str(temp))
def toDisk(dest_dir, dest_num, frame):

    plt.figure()
    plt.imsave(dest_dir+'/VIGIA_IR_'+dest_num+'.png',frame)
    plt.imshow(frame)
    plt.show()
    return(0)

def makeColored(image, metad, scale=True):
    #time_rec = time.gmtime(metad['timestamp']/tstamp_factor)
    time_rec = time.gmtime(metad[2]/tstamp_factor)
    ...
    if scale: frame[344:600,12:24] = gen_colorbar()
...
if __name__ == '__main__':
    ...
    frame = makeColored(image,metad)
    # save image to dest_dir in disk
    toDisk(dest_dir, dest_num, frame)
